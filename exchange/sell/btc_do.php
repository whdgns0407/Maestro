<?php
	...
	else
	{
		...
		echo "<meta http-equiv=refresh content='0 url=./sellbtc.php'>";
		exit();
	}
	date_default_timezone_set('Asia/Seoul');
	$date = date("Y-m-d H:i:s");
	$btcamount = $_POST['btcamount'];
	$sum = floor($btcamount * $buy[1]);
	...
	if ($btcamount <= $queryinformation['btc'])
	{
	...
	{
		echo "<script>alert('비트코인 개수가 맞지 않아, 판매가 불가능 합니다. 재시도하여 주십시오.')</script>";
		echo "<meta http-equiv=refresh content='0 url=./sellbtc.php'>";
		exit();
	}
?>
