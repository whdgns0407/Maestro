	...
	<body>
		<?php
			...
			if(isset($_SESSION['email']))
			{
				...
				$bnoquery = mysql_query("select * from reply WHERE idx='$bno'");
				$bnoinformation = mysql_fetch_array($bnoquery);
				
				if($queryinformation['email'] == $bnoinformation['pw'])
				{
					$sql = mysql_query("delete from reply where idx='$bno';");
				...
				{
					echo "<script>alert('현재 로그인된 아이디와 덧글 작성 아이디가 다릅니다.')</script>";
					echo "<meta http-equiv=refresh content='0 url=./read.php?idx=$bbno'>";
					exit();
				}
			}
